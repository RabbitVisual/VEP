...
class BibleApiService
{
     ...
     *
     * @return Collection<int, BibleBook>
     */
    public function getBooks(?int $versionId = null): Collection
    {
        ...
        return Cache::remember(self::CACHE_PREFIX."books_{$versionId}", self::CACHE_TTL_SECONDS, function () use ($versionId) {
            return BibleBook::where('bible_version_id', $versionId)
                ->orderBy('book_number')
     ...
     *
     * @return Collection<int, BibleChapter>
     */
    public function getChapters(?int $bookId = null, ?string $bookName = null, ?int $versionId = null): Collection
    {
        if ($bookId) {
            return Cache::remember(self::CACHE_PREFIX."chapters_{$bookId}", self::CACHE_TTL_SECONDS, function () use ($bookId) {
                return BibleChapter::where('book_id', $bookId)
        ...
        if ($bookName && $versionId) {
            $book = BibleBook::where('bible_version_id', $versionId)
                ->where('name', $bookName)
                ->first();
            if ($book) {
    ...
    public function getVerses(
        ...
        $query = BibleVerse::where('chapter_id', $chapterId)->orderBy('verse_number');
    ...
    public function findByReference(string $reference): ?array
    {
        ...
        $chapter = BibleChapter::where('book_id', $book->id)
    ...
    public function search(string $query, int $limit = 10): Collection
    {
        ...
        return BibleVerse::search($query)
            ->take($limit)
            ->get();
    }
     ...
     */
    public function getRandomVerse(?int $versionId = null): ?BibleVerse
    {
        $q = BibleVerse::with('chapter.book')->inRandomOrder();
    ...
    public function compare($v1, $v2, int $bookNumber, int $chapter, ?int $verse = null): ?array
    {
        ...
        $book1 = BibleBook::where('bible_version_id', $version1->id)->where('book_number', $bookNumber)->first();
        $book2 = BibleBook::where('bible_version_id', $version2->id)->where('book_number', $bookNumber)->first();

        if (! $book1 || ! $book2) {
            return null;
        }

        $query1 = BibleVerse::select('bible_verses.*')
            ->join('bible_chapters', 'bible_verses.chapter_id', '=', 'bible_chapters.id')
            ->where('bible_chapters.book_id', $book1->id)
            ->where('bible_chapters.chapter_number', $chapter);

        $query2 = BibleVerse::select('bible_verses.*')
            ->join('bible_chapters', 'bible_verses.chapter_id', '=', 'bible_chapters.id')
            ->where('bible_chapters.book_id', $book2->id)
            ->where('bible_chapters.chapter_number', $chapter);

        if ($verse !== null) {
            $query1->where('bible_verses.verse_number', $verse);
            $query2->where('bible_verses.verse_number', $verse);
        } else {
            $query1->orderBy('bible_verses.verse_number');
            $query2->orderBy('bible_verses.verse_number');
        }
