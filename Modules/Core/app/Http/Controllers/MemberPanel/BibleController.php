...
use VertexSolutions\Core\Models\BibleVerse as Verse;
...
class BibleController extends Controller
{
    ...
    public function performSearch(Request $request)
    {
        ...
        $verses = Verse::where('bible_verses.text', 'LIKE', "%{$query}%")
            ->join('bible_chapters', 'bible_verses.chapter_id', '=', 'bible_chapters.id')
            ->join('bible_books', 'bible_chapters.book_id', '=', 'bible_books.id')
            ->select(
                'bible_verses.id',
                'bible_verses.text',
                'bible_verses.verse_number as verse_number',
                'bible_chapters.chapter_number as chapter_number',
                'bible_books.name as book_name',
                'bible_books.book_number as book_number'
            )
            ->limit(50)
            ->get();
    ...
    public function favorites()
    {
        ...
        $favorites = Verse::join('bible_favorites', 'bible_verses.id', '=', 'bible_favorites.verse_id')
            ->where('bible_favorites.user_id', $user->id)
            ->with(['chapter.book.bibleVersion'])
            ->select('bible_verses.*', 'bible_favorites.color as favorite_color')
            ->get();
