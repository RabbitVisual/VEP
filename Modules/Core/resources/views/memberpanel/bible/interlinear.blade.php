    ...
    <header class="sticky top-0 z-40 bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl border-b border-gray-200 dark:border-slate-800 transition-colors">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 sm:h-20 flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4">
                <a href="{{ route('painel.bible.index') }}"
...
<script>
(function () {
    var base = @json(rtrim(route('painel.bible.interlinear'), '/'));
    var baseDir = base.replace(/\/interlinear$/, '');
    ...
    return {
        ...
        async loadBooks() {
            ...
            try {
                const res = await fetch(baseDir + '/interlinear/books');
        ...
        async loadData() {
            ...
            try {
                const url = baseDir + '/interlinear/data?book=' + encodeURIComponent(this.selectedBook) + '&chapter=' + this.selectedChapter + '&testament=' + this.selectedTestament;
        ...
        async showStrong(number, ptLemma = null, ptSuggestion = null) {
            ...
            try {
                const res = await fetch(baseDir + '/strong/' + encodeURIComponent(cleanNum));
