...
@section('content')
@php
    $searchChapterUrlBase = isset($versions) && $versions->isNotEmpty()
        ? url('/painel/bible/chapter/' . $versions->first()->abbreviation)
        : url('/painel/bible/chapter/NVI');
@endphp
<div class="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors pb-20"
     data-chapter-url-base="{{ $searchChapterUrlBase }}"
     x-data="bibleSearch()"
     x-init="chapterUrlBase = $el.getAttribute('data-chapter-url-base') || ''">

    @include('bible::memberpanel.bible.partials.bible-header-assets')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-4 space-y-6">
        @include('bible::memberpanel.bible.partials.bible-reader-bar', [
            'title' => 'Busca Bíblica',
            'subtitle' => 'Concordância · atalhos rápidos',
            'showBack' => true,
            'backUrl' => route('painel.bible.index'),
            'showVersionSelector' => false,
            'showSearch' => false,
            'showFavorites' => true,
            'showInterlinear' => true,
            'showLeitura' => true,
            'version' => isset($versions) && $versions->isNotEmpty() ? $versions->first() : null,
        ])
        <div>
            <!-- Search Bar -->
            <div class="relative group max-w-3xl" data-tour="bible-search-input">
        ...
        <div x-show="results.length === 0 && !hasSearched" class="text-center py-24 opacity-60">
            <div class="w-24 h-24 bg-gray-100 dark:bg-slate-900 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner">
                <x-icon name="book-open" style="duotone" class="w-10 h-10 text-gray-300 dark:text-slate-700" />
            </div>
        ...
        <div class="space-y-4" data-tour="bible-search-results">
            <template x-for="verse in results" :key="verse.id">
                <a :href="chapterUrlBase + '/' + verse.book_number + '/' + verse.chapter_number + '#verse-' + verse.verse_number"
                   class="block bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-slate-800 hover:border-indigo-500 dark:hover:border-indigo-500 shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 transition-all group relative overflow-hidden">
                    ...
                    <p class="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mt-1">{{ isset($versions) && $versions->isNotEmpty() ? $versions->first()->abbreviation : 'NVI' }}</p>
                             </div>
                        </div>
...
<script>
    function bibleSearch(opts) {
        opts = opts || {};
        return {
            chapterUrlBase: opts.chapterUrlBase || '',
            query: '',
            results: [],
            loading: false,
            hasSearched: false,
