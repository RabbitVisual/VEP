...
@section('content')
    <div class="p-6 space-y-6">
        ...
        <!-- Concordância Strong (Interlinear / NEPE) -->
        <div class="mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex items-center justify-between flex-wrap gap-3">
            <div class="flex items-center gap-3">
                <x-module-icon alias="core" class="w-8 h-8 text-indigo-600 dark:text-indigo-400" />
                <div>
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ module_nav_label('core') }} – Concordância Strong</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-0.5">Editar e refinar textos, explicações e contexto (lemma_br, descrição, DITAT/TWOT, Gematria). Estilo NEPE.</p>
                </div>
            </div>
            <a href="{{ route('admin.bible.strong.index') }}"
                class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition-colors focus:ring-4 focus:ring-indigo-300 dark:focus:ring-indigo-800">
                <x-icon name="book" style="duotone" class="w-5 h-5 mr-2" />
                Abrir editor Strong
            </a>
        </div>
