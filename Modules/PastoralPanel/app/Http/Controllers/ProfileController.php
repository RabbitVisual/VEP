...
class ProfileController extends Controller
{
    public function show(): View
    {
        ...
        return view('pastoralpanel::profile.show', [
            'user' => $user,
    ...
    {
        $user = auth()->user();
        return view('pastoralpanel::profile.edit', ['user' => $user]);
    }
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('pastoral.profile.show')->with('success', 'Perfil atualizado.');
    }
    ...
    public function requestSensitiveChange(Request $request): RedirectResponse
    {
        ...
        $exists = SensitiveFieldChangeRequest::where('user_id', $user->id)
            ...
            ->exists();
        if ($exists) {
            return redirect()->route('pastoral.profile.show')->with('error', 'Este campo já possui uma solicitação (uma única vez por campo).');
        }
        ...
        return redirect()->route('pastoral.profile.show')->with('success', 'Solicitação de alteração enviada. Aguarde aprovação do administrador.');
    }
}
