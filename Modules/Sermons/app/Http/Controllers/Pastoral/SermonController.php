...
use VertexSolutions\Sermons\Jobs\NotifyMembersOfNewContent;
...
class SermonController extends Controller
{
    ...
    public function edit(Sermon $sermon): View
    {
        ...
        $worshipSongs = class_exists(\VertexSolutions\Worship\App\Models\WorshipSong::class)
            ? \VertexSolutions\Worship\App\Models\WorshipSong::orderBy('title')->get(['id', 'title'])
            : collect();
    ...
    public function inviteCollaborator(Request $request, Sermon $sermon): RedirectResponse
    {
        ...
        $collab = $sermon->collaborators()->create([
            'user_id' => $user->id,
            'role' => \VertexSolutions\Sermons\Models\SermonCollaborator::ROLE_EDITOR,
            'can_edit' => true,
            'status' => \VertexSolutions\Sermons\Models\SermonCollaborator::STATUS_PENDING,
            'invited_at' => now(),
        ]);
        if (class_exists(\VertexSolutions\Notifications\App\Services\InAppNotificationService::class)) {
            app(\VertexSolutions\Notifications\App\Services\InAppNotificationService::class)->sendToUser(
                ...
                ['action_url' => route('painel.sermons.collaborator.invite', $collab), 'action_text' => 'Ver convite', 'notification_type' => 'sermon_collaboration']
            );
        }
    ...
    public function exportPdf(Request $request, Sermon $sermon): Response
    {
        ...
        $topicsFromFullContentHtml = ($format === 'topics' && $sermon->full_content) ? $this->extractTopicsFromHtml($mark($sermon->full_content)) : '';
