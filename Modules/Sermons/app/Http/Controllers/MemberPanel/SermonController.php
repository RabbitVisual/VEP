...
use VertexSolutions\Sermons\Models\SermonCollaborator;
use VertexSolutions\Sermons\Models\SermonFavorite;
use VertexSolutions\Sermons\Models\SermonTag;
...
class SermonController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('painel.sermons.show', $sermon)
    ...
    public function edit(Sermon $sermon): View
    {
        ...
        $categories = SermonCategory::active()->ordered()->get();
        $tags = SermonTag::all();
        $series = BibleSeries::where('status', 'published')->orderBy('title')->get();
    ...
    public function update(Request $request, Sermon $sermon): RedirectResponse
    {
        ...
        if ($request->has('bible_references') && is_array($request->bible_references)) {
            ...
            $booksMap = $bookIds->isNotEmpty() ? BibleBook::whereIn('id', $bookIds)->pluck('name', 'id') : collect();
            $chaptersMap = $chapterIds->isNotEmpty() ? BibleChapter::whereIn('id', $chapterIds)->pluck('chapter_number', 'id') : collect();
            foreach ($request->bible_references as $index => $ref) {
        ...
        return redirect()->route('painel.sermons.show', $sermon)->with('success', 'Sermão atualizado com sucesso!');
    }
    ...
    public function destroy(Sermon $sermon): RedirectResponse
    {
        ...
        $sermon->delete();
        return redirect()->route('painel.sermons.my-sermons')->with('success', 'Sermão excluído.');
    }
    ...
    public function showCollaboratorInvite(SermonCollaborator $collaborator): View|RedirectResponse
    {
        ...
        if ($collaborator->status !== SermonCollaborator::STATUS_PENDING) {
            return redirect()->route('painel.sermons.show', $collaborator->sermon)
    ...
    public function respondCollaborator(Request $request, SermonCollaborator $collaborator): RedirectResponse
    {
        ...
        if ($collaborator->status !== SermonCollaborator::STATUS_PENDING) {
            return redirect()->route('painel.sermons.index')->with('info', 'Convite já respondido.');
        }
        ...
        return redirect()->route('painel.sermons.show', $collaborator->sermon)->with('success', $message);
    }
    ...
    public function exportPdf(Request $request, Sermon $sermon): Response
    {
        ...
        if ($format === 'topics' && $sermon->full_content) {
            $topicsFromFullContentHtml = $this->extractTopicsFromHtml($mark($sermon->full_content));
        }
