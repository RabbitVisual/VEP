...
class SermonController extends Controller
{
    ...
    public function create(): View
    {
        $categories = SermonCategory::active()->ordered()->get();
        $tags = SermonTag::all();
        $series = \VertexSolutions\Sermons\Models\BibleSeries::where('status', 'published')->orderBy('title')->get();
        $worshipSongs = \VertexSolutions\Worship\App\Models\WorshipSong::orderBy('title')->get(['id', 'title']);
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($request->has('bible_references') && is_array($request->bible_references)) {
            ...
            $booksMap = $bookIds->isNotEmpty()
                ? \VertexSolutions\Core\Models\Book::whereIn('id', $bookIds)->pluck('name', 'id')
                : collect();

            $chaptersMap = $chapterIds->isNotEmpty()
                ? \VertexSolutions\Core\Models\Chapter::whereIn('id', $chapterIds)->pluck('chapter_number', 'id')
                : collect();
    ...
    public function inviteCollaborator(Request $request, Sermon $sermon): RedirectResponse
    {
        ...
        if (class_exists(\VertexSolutions\Notifications\App\Services\InAppNotificationService::class)) {
            app(\VertexSolutions\Notifications\App\Services\InAppNotificationService::class)->sendToUser(
                ...
                [
                    'action_url' => route('painel.sermons.collaborator.invite', $collab),
