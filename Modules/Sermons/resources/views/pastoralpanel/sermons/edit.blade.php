...
@section('content')
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <form id="sermon-form" action="{{ route('pastor.sermoes.sermons.update', $sermon) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <!-- Tags -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6" x-data="{ aiSuggestLoading: false, aiSuggestions: null, aiError: null }">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">Tags</h3>
                    @if(Route::has('sermons.ai.suggest'))
                    <button type="button"
                        @click="aiSuggestLoading = true; aiError = null; aiSuggestions = null; fetch('{{ route('sermons.ai.suggest', $sermon) }}', { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content } }).then(r => r.json()).then(d => { aiSuggestions = d; aiSuggestLoading = false; }).catch(() => { aiError = 'Erro ao sugerir.'; aiSuggestLoading = false; })"
                        :disabled="aiSuggestLoading"
                        class="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-lg bg-primary/10 text-primary hover:bg-primary/20 disabled:opacity-50">
                        <x-icon name="sparkles" style="duotone" class="w-4 h-4" />
                        <span x-text="aiSuggestLoading ? 'Sugerindo...' : 'Sugerir com IA'"></span>
                    </button>
                    @endif
                </div>
                <div x-show="aiSuggestions" x-cloak class="mb-4 p-4 rounded-xl bg-primary/5 border border-primary/20 dark:border-primary/30">
                    <p class="text-xs font-bold text-primary uppercase tracking-wide mb-2">Sugestões da IA</p>
                    <div class="flex flex-wrap gap-2 mb-2" x-show="aiSuggestions && aiSuggestions.suggested_tags && aiSuggestions.suggested_tags.length">
                        <template x-for="tag in (aiSuggestions && aiSuggestions.suggested_tags) || []" :key="tag">
                            <button type="button" @click="document.getElementById('tag-input').value = tag; document.getElementById('add-tag').click();"
                                class="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-primary/15 text-primary hover:bg-primary/25" x-text="tag"></button>
                        </template>
                    </div>
                    <div class="text-xs text-gray-600 dark:text-gray-400" x-show="aiSuggestions && aiSuggestions.suggested_references && aiSuggestions.suggested_references.length">
                        <span class="font-medium">Referências:</span>
                        <ul class="mt-1 list-disc list-inside">
                            <template x-for="ref in (aiSuggestions && aiSuggestions.suggested_references) || []" :key="ref.reference_text">
                                <li x-text="ref.reference_text + (ref.type ? ' (' + ref.type + ')' : '')"></li>
                            </template>
                        </ul>
                        <p class="mt-2 text-gray-500">Adicione-as manualmente na seção de referências bíblicas abaixo.</p>
                    </div>
                </div>
                <p x-show="aiError" class="mb-4 text-sm text-red-600 dark:text-red-400" x-text="aiError"></p>
