        ...
        <div class="mt-16 pt-8 border-t border-gray-100 dark:border-gray-800">
            <div class="flex flex-wrap gap-2 mb-8">
                @foreach($sermon->tags as $tag)
                    <a href="{{ route('painel.sermons.index', ['tag_id' => $tag->id]) }}" class="text-sm text-gray-500 hover:text-amber-500 dark:text-gray-400 dark:hover:text-amber-400 transition-colors">
                        #{{ $tag->name }}
                    </a>
                @endforeach
            </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex space-x-4">
                    <form action="{{ route('painel.sermons.toggle-favorite', $sermon->id) }}" method="POST">
                        @csrf
                ...
                @if($canEdit)
                    <a href="{{ route('painel.sermons.edit', $sermon) }}" class="inline-flex items-center min-h-[44px] py-2 text-sm font-bold text-blue-500 hover:text-blue-600 touch-manipulation">
                        Editar Sermão
                    </a>
                        ...
                        <div x-show="exportModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" x-transition>
                            <div @click.outside="exportModalOpen = false" class="w-full max-w-md rounded-xl bg-white dark:bg-gray-800 shadow-xl border border-gray-200 dark:border-gray-700 p-6">
                                ...
                                <div class="mt-6 flex gap-3 justify-end">
                                    ...
                                    <a :href="'{{ route('painel.sermons.export-pdf', $sermon) }}?format=' + format + '&size=' + size" target="_blank" @click="exportModalOpen = false"
