...
@section('title', 'Criar curso – Academia')
...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-lg font-semibold text-slate-100">Novo curso</h2>
    </div>

    <form action="{{ route('pastoral.academy.courses.store') }}" method="POST" enctype="multipart/form-data" class="max-w-2xl space-y-6 rounded-xl border border-slate-700 bg-slate-800/50 p-6">
        @csrf
        ...
        <div>
            <label for="slug" class="block text-sm font-medium text-slate-300">Slug (opcional)</label>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" class="mt-1 block w-full rounded-lg border-slate-600 bg-slate-800 text-slate-100 shadow-sm focus:ring-indigo-500">{{ old('description') }}</textarea>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <select name="level" id="level" class="mt-1 block w-full rounded-lg border-slate-600 bg-slate-800 text-slate-100">
                    <option value="iniciante" @selected(old('level', 'iniciante') === 'iniciante')>Iniciante</option>
                    <option value="intermediário" @selected(old('level') === 'intermediário')>Intermediário</option>
                    <option value="avançado" @selected(old('level') === 'avançado')>Avançado</option>
                </select>
            </div>
            <div>
        ...
        <div class="flex gap-3">
            <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">Criar curso</button>
            <a href="{{ route('pastoral.academy.courses.index') }}" class="rounded-lg border border-slate-600 px-4 py-2 text-sm font-medium text-slate-300 hover:bg-slate-700">Cancelar</a>
        </div>
    </form>
</div>
@endsection
