...
@section('title', 'Nova aula – Academia')
...
<div class="space-y-6">
    ...
    <form action="{{ route('pastoral.academy.lessons.store', $module) }}" method="POST" class="space-y-6 rounded-xl border border-slate-700 bg-slate-800/50 p-6">
        ...
        <div>
            <label for="video_url" class="block text-sm font-medium text-slate-300">URL do vídeo (YouTube / Vimeo / embed)</label>
            <input type="url" name="video_url" id="video_url" value="{{ old('video_url') }}" placeholder="https://..."
                   class="mt-1 block w-full rounded-lg border-slate-600 bg-slate-800 text-slate-100 shadow-sm focus:ring-indigo-500">
            @error('video_url')<p class="mt-1 text-sm text-red-400">{{ $message }}</p>@enderror
        </div>

        <div>
            <label for="content" class="block text-sm font-medium text-slate-300">Conteúdo (texto / material de apoio)</label>
            <x-rich-editor name="content" :value="old('content')" placeholder="Use @ para inserir referências bíblicas." />
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                ...
                <input type="number" name="duration_in_minutes" id="duration_in_minutes" value="{{ old('duration_in_minutes', 0) }}" min="0"
                       class="mt-1 block w-full rounded-lg border-slate-600 bg-slate-800 text-slate-100 shadow-sm focus:ring-indigo-500">
            </div>
            <div>
        ...
        <div class="flex gap-3">
            <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">Criar aula</button>
            <a href="{{ route('pastoral.academy.courses.show', $module->course) }}" class="rounded-lg border border-slate-600 px-4 py-2 text-sm font-medium text-slate-300 hover:bg-slate-700">Cancelar</a>
        </div>
    </form>
</div>
@endsection
