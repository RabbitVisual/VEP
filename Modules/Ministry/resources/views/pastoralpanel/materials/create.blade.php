...
@section('title', 'Novo material – ' . $ministry->name)
...
@section('content')
    <div class="p-6 space-y-6">
        ...
        <div class="backdrop-blur-xl bg-white/70 dark:bg-white/10 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 max-w-3xl">
            <form action="{{ route('ministry.materials.store', $ministry) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="8" data-mention-editor="true"
                        class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white sm:text-sm @error('content') border-red-500 @enderror"
                        placeholder="Texto do material (suporta @menções bíblicas)">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <a href="{{ route('ministry.materials.index', $ministry) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800">
                        Cancelar
                    </a>
