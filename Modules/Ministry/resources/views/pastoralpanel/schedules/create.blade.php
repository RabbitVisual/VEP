...
@section('title', 'Nova escala – ' . $ministry->name)
    ...
    <div class="p-6 space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Nova escala</h1>
            </div>
        </div>
        ...
        <div class="backdrop-blur-xl bg-white/70 dark:bg-white/10 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 max-w-2xl">
            <form action="{{ route('ministry.schedules.store', $ministry) }}" method="POST" class="space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        ...
                        placeholder="Observações ou instruções">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <a href="{{ route('ministry.schedules.index', $ministry) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700">
                        Cancelar
                    </a>
