<x-memberpanel::layouts.master title="Meu perfil">
    <div class="p-6 space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4">
                @if ($user->avatar_url)
                    <img src="{{ $user->avatar_url }}" alt="" class="size-16 shrink-0 rounded-full object-cover ring-2 ring-gray-200 dark:ring-slate-700">
                @else
                    <div class="flex size-16 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400">
                        <x-icon name="user" style="duotone" class="size-8" />
                    </div>
                @endif
                <div class="space-y-1">
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Meu perfil</h1>
            ...
            <a href="{{ route('painel.profile.edit') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-700">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            {{-- Dados sensíveis (somente leitura; solicitar alteração 1x por campo) --}}
            <div class="backdrop-blur-xl bg-white/70 dark:bg-white/10 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="border-b border-gray-200 dark:border-gray-700 p-4">
    ...
    {{-- Modais: solicitar alteração (1 por campo) --}}
    @foreach (App\Models\SensitiveFieldChangeRequest::SENSITIVE_FIELDS as $field)
